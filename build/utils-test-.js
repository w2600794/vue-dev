...
// glob 是webpack 安装时依赖的一个第三方模块 ，模块允许使用*等符号， eg: lib/*.js 就是获取lib文件下的所有js后缀名的文件
...
/**********************测试***************************/
var template_PATH = path.resolve(__dirname, '../src/web-template');    //模板路径
...
exports.htmlPlugin = function () {
    let entryHtml = glob.sync(PAGE_PATH + '/**/**/**/**/*.vue');           // 页面路径
    let templateHtml = glob.sync(template_PATH + '/**/**/**/**/*.html');           // 页面路径
    let arr = [];
    console.log()
    ...
    entryHtml.forEach((filePath, i) => {
        ...
        let conf = {
            ...
            // 文件名称
            filename: fileFolder==''?(filename + '.html'):(fileFolder + '/'+ filename + '.html'),
